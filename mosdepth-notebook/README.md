![docker pulls](https://img.shields.io/docker/pulls/marcsaric/mosdepth-notebook.svg) ![docker stars](https://img.shields.io/docker/stars/marcsaric/mosdepth-notebook.svg) [![](https://images.microbadger.com/badges/image/marcsaric/mosdepth-notebook.svg)](https://microbadger.com/images/marcsaric/mosdepth-notebook "marcsaric/mosdepth-notebook image metadata")

# Fork of Jupyter Notebook Data Science Stack

This is fork of the [jupyter/datascience-notebook](https://github.com/jupyter/docker-stacks/tree/master/datascience-notebook) stack adding some modifications, specifically a full Biopython install (more or less).
...
```
docker run -it --rm -p 8888:8888 marcsaric/mosdepth-notebook
```
...
```
docker run -d -p 8888:8888 marcsaric/mosdepth-notebook start-notebook.sh --NotebookApp.password='sha1:74ba40f8a388:c913541b7ee99d15d5ed31d4226bf7838f83a50e'
```

For example, to set the base URL of the notebook server, run the following:

```
docker run -d -p 8888:8888 marcsaric/mosdepth-notebook start-notebook.sh --NotebookApp.base_url=/some/path
```

For example, to disable all authentication mechanisms (not a recommended practice):

```
docker run -d -p 8888:8888 marcsaric/mosdepth-notebook start-notebook.sh --NotebookApp.token=''
```
...
docker run -d -p 8888:8888 \
    ...
    marcsaric/mosdepth-notebook start-notebook.sh \
...
```
docker run -it --rm marcsaric/mosdepth-notebook start.sh ipython
```

Or, to run JupyterLab instead of the classic notebook, run the following:

```
docker run -it --rm -p 8888:8888 marcsaric/mosdepth-notebook start.sh jupyter lab
```
